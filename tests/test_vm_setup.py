...
class TestVMSetupAPIKey:
    ...
    def test_setup_api_key_writes_to_bashrc(self, mock_file, mock_home, vm_setup, tmp_path, capsys):
...
class TestVMSetupGitHubCLI:
    ...
    @patch('vm_setup_script.Path.home')
    @patch('builtins.open', new_callable=mock_open)
    def test_setup_github_cli(self, mock_file, mock_home, mock_system, vm_setup, tmp_path):
...
class TestVMSetupSSHKeys:
    ...
    @patch('vm_setup_script.Path.home')
    @patch('vm_setup_script.CommandRunner.run')
    def test_setup_ssh_keys_skip_existing(self, mock_run, mock_home, mock_input, vm_setup, tmp_path, capsys):
    ...
    @patch('vm_setup_script.Path.home')
    @patch('vm_setup_script.CommandRunner.run')
    def test_setup_ssh_keys_with_pub_key(self, mock_run, mock_home, mock_system, vm_setup, tmp_path, capsys):
...
class TestVMSetupJuggle:
    ...
    @patch('vm_setup_script.CommandRunner.run')
    def test_setup_juggle_copies_config(self, mock_run, mock_path_class, mock_home, mock_copy, vm_setup, tmp_path, capsys):
        ...
        mock_path_class.side_effect = lambda x: mock_path_instance if str(x) == str(tmp_path / '.juggle') else mock_path_instance
...
class TestStartRalphLoop:
    ...
    @patch('vm_setup_script.CommandRunner.run')
    def test_start_ralph_loop_with_spec(self, mock_run, mock_cwd, vm_setup, tmp_path, capsys):
    ...
    @patch('vm_setup_script.Path.cwd')
    @patch('vm_setup_script.CommandRunner.run')
    def test_start_ralph_loop_without_spec(self, mock_run, mock_cwd, vm_setup, tmp_path, capsys):
    ...
    @patch('vm_setup_script.Path.cwd')
    @patch('vm_setup_script.CommandRunner.run')
    def test_start_ralph_loop_default_session(self, mock_run, mock_cwd, vm_setup, tmp_path):
...
class TestRunFullSetup:
    ...
    def test_run_full_setup_calls_all_methods(self, mock_uv, mock_juggle, mock_opencode, vm_setup, capsys):
    ...
    @patch('vm_setup_script.VMSetup.setup_juggle')
    @patch('vm_setup_script.VMSetup.setup_uv_pip')
    def test_run_full_setup_handles_exception(self, mock_uv, mock_juggle, mock_opencode, vm_setup, capsys):
...
class TestMainFunction:
    ...
    def test_main_requires_environment_variables(self):
        ...
        with patch.dict(os.environ, {'GIT_EMAIL': '{{test@', 'OPENROUTER_API_KEY': '{{key}'}, clear=False):
            with patch('sys.exit'):
    ...
    def test_main_with_valid_env_vars(self):
        ...
        with patch.dict(os.environ, {'GIT_EMAIL': 'test@test.com', 'OPENROUTER_API_KEY': 'valid-key'}, clear=False):
        ...
        with patch('vm_setup_script.Path.home') as mock_home:
            ...
            with patch('builtins.open', mock_open()) as mock_file:
                ...
                for call in mock_file().write.call_args_list:
                    ...
                    assert "SUPER_SECRET_API_KEY_12345" not in content or "export OPENROUTER_API_KEY=" in content
